...
function fillSpace(i, ips = []) {
	let previousCoo = { x: 0, y: 0 };
	let length = 0;

	for (let j = 0; j < wires[i].length; j++) {
		const operation = wires[i][j];
		const operationOp = operation.substr(0, 1);
		const operationData = +operation.substr(1);

		const add = { x: 0, y: 0, sign: 1 };
		...
		for (let o = 1; o <= operationData; o++) {
			length++;

			const coo = {
				x: previousCoo.x + add.x,
				y: previousCoo.y + add.y
			};
			previousCoo = coo;
			const jcoo = JSON.stringify(coo);

			if (space[jcoo] === undefined) {
				space[jcoo] = [i, length];
			} else if (space[jcoo][0] !== i) {
				ips.push([coo, length + space[jcoo][1]]);
			}
		}
	}
}
...
const wires = inputs.map(wire => wire.split(","));
...
const closestFromCenter = ips
	.map(([e]) => Math.abs(e.x) + Math.abs(e.y))
	.sort((a, b) => a - b)[0];

const closestInDistance = ips.map(([, l]) => l).sort((a, b) => a - b)[0];

console.log(
	"closestFromCenter, closestInDistance",
	closestFromCenter,
	closestInDistance
);
